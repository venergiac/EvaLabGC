...
namespace EvaLab.EOG
{
    ...
    public class ExperimentRunner
    {
        ...
        public double HotTestStart()
        {
            ...
            Debug.WriteLine("HotTest STOP");
            return totTime / ((double)(TOTS-SKIP));
        }
        ...
        public virtual void Start()
        {
            ...
            if (!(msReal <= msToWait))
            {
                DialogResult result = MessageBox.Show("Warning acquisition is lossing data. Millisecond to wait " + msToWait + " real ms " + msReal, "Acquisition is lossing data", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
                if (result == DialogResult.Cancel)
                {
                    throw new Exception("Acquisition interrupted according to user request");
                }
            }
        ...
        {
            Debug.WriteLine("Runner Stop call");
            try
            {
                running = false;
                workerThread.Abort();
            ...
            finally
            {
                if (EventRunner != null)
                {
                    EventRunner(this, new ExperimentRunnerEventArgs(ExperimentRunnerEventArgs.EXPERIMENT_STOP, time));
                }
            }
        }
        ...
        private void Run()
        {
            try
            {
                ...
                Debug.WriteLine("ExperimentRunner: FINISH");
            }
            catch (Exception e)
            {
                Debug.WriteLine(e);
            ...
            {
                Debug.WriteLine("ExperimentRunner: STOP after finally on Run");
                Stop();
            }
        }
