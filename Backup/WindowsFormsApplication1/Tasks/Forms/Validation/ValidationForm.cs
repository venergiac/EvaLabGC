    ...
    public class ValidationForm : Form1
    {
        ...
        private Image MakeImageResult()
        {
            ...
            for (int i = 0; i < nPoint; i++)
            {
                ...
                g.DrawLine(new Pen(Color.Green,3), x[i], y[i], (float)((double)this.ClientSize.Width * xv[i] / (double)Reader.ASLWIDTH), (float)((double)this.ClientSize.Height * yv[i] / (double)Reader.ASLHEIGHT));
            }
        ...
        {
            return Math.Sqrt(Math.Pow(x[i] - (float)((double)this.ClientSize.Width * xv[i] / (double)Reader.ASLWIDTH), 2) + Math.Pow(y[i] - (float)((double)this.ClientSize.Height * yv[i] / (double)Reader.ASLHEIGHT), 2));
        }
