...
namespace EVALab.Analysis.Data
{
    public class DataManager
    {
        ...
        private DataList AdjustReferenceByType(string type, DataList list, decimal maxX, decimal maxY, decimal scaleX, decimal scaleY)
        {
            ...
            {
                return AdjustSingleReference(list);
            }
            else if (type.Equals("XY_XNA"))
            {
                return AdjustSingleReference(list);
            }
            else
        ...
        private DataList AdjustSingleReference(DataList list)
        {
            foreach (Item item in list.List)
            {
                double refInt = item.Reference[0];
                item.Reference[0] = (double)((int)(refInt / 10000));
                item.Reference[1] = (double)((int)(refInt % 10000));
            }
