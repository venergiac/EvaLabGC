...
namespace EVALab.Analysis.WaveForm
{
    public class WaveFormManager
    {
        ...
        {
            saccade.IdxEnd = Math.Min(saccade.IdxEnd, data.List.Count-1);
            ...
            else
            {
                saccade.Amplitude = 
                    Math.Sqrt(
                    Math.Pow(data.List[saccade.IdxEnd].Value[Item.POSITIONX] - data.List[saccade.IdxStart].Value[Item.POSITIONX],2) +
                    Math.Pow(data.List[saccade.IdxEnd].Value[Item.POSITIONY] - data.List[saccade.IdxStart].Value[Item.POSITIONY], 2));
            }
