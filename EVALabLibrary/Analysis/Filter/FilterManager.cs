...
namespace EVALab.Analysis.Filter
{
    public class FilterManager
    {
        ...
        //second order
        public DataList Butterworth2Order50Hz(DataList input, WaveForm.WaveFormType typeFilter, bool scale)
        {
            return Butterworth(input, new double[] { -0.8093181379, -1.7891095492 }, 1.111596605, typeFilter,scale);
        }

        public DataList Butterworth(DataList input, double[] zCoeff, double gain, WaveForm.WaveFormType typeFilter, bool scale)
        {
            ...
            {
                Item item2 = item.Clone();
                // filter according to waveform
                if ((typeFilter.Equals(WaveFormType.None)) ||
                    (IsInWaveform(idx, input, typeFilter) != null))
                {
            ...
            return (scale) ? Scale(output, input) : output;
        }

        public DataList Median(DataList input, int order, WaveForm.WaveFormType typeFilter, bool scale)
        {
            DataList output = DataList.NewInstance(input);
            output.Name += " median denoised";

            Median filterX = new Median(order);
            Median filterY = new Median(order);
        ...
        public DataList Denoise(DataList input, int order, WaveForm.WaveFormType typeFilter, bool scale)
        {
            DataList output = DataList.NewInstance(input);
        ...
        public DataList Filter(DataList input, int cutOff, int order, WaveFormType typeFilter, bool scale)
        {
            DataList output = DataList.NewInstance(input);
            output.Name += " filtered";
        ...
        private WaveForm.WaveForm IsInWaveform(int idx, DataList list, WaveFormType typeFilter)
        {
            //saccade
            if (typeFilter.Equals(WaveFormType.Saccade))
            {
                foreach (WaveForm.WaveForm item in list.Saccades.List)
                {
                    if ((item.Type.Equals(typeFilter)) || (typeFilter.Equals(WaveFormType.None)))
                    {
                        if ((idx >= item.IdxStart) && (idx <= item.IdxEnd)) return item;
                    }
                }
            }
            else
            {
                //waveform
                foreach (WaveForm.WaveForm item in list.WaveForms.List)
                {
                    if ((item.Type.Equals(typeFilter)) || (typeFilter.Equals(WaveFormType.None)))
                    {
                        if ((idx >= item.IdxStart) && (idx <= item.IdxEnd)) return item;
