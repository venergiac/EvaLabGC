...
namespace EVALabGC.Object
{
    public class TaskManager
    {
        ...
        public Task AddAndParseFileTask(string filename, Stream stream)
        {
            try
            {
                Task task = ParseFileTask(filename, stream);
                tasks.Add(task);
                return task;
        ...
        public Task ParseFileTask(string filename, Stream stream)
        {
        ...
        public void ReloadTasks()
        {
            ...
            foreach(Task t in tasks) {
                ...
                try
                {
                    filename=((string)t.Map["file"]);
                    file = new FileStream(filename, FileMode.Open);
                    Task newtask = TaskManager.Instance().ParseFileTask(filename, file);
                    tasksNew.Add(newtask);
                }
                catch(Exception ex)
                {
                    ExceptionForm.Show(null, "Error on file " + filename, ex);
                }
                finally
                {
        ...
        {
            return new Task(Task.TaskTypeId.Calibration.ToString(), Task.TaskTypeId.Calibration.ToString(), "Calibration test: use this test to calibrate the Tracking Machine.", new Hashtable(), false, false);
        }


        public Task MakeTestTask()
        {
            return new Task(Task.TaskTypeId.Test.ToString(), Task.TaskTypeId.Test.ToString(), "Validation test: see the eye position in real time.", new Hashtable(), true, false);
        }

        public Task MakeValidationTask()
        {
            return new Task(Task.TaskTypeId.Validation.ToString(), Task.TaskTypeId.Validation.ToString(), "Validation test: use this test to validate the precision of Tracking Machine.", new Hashtable(), true, false);
        }
    }
}
