...
namespace EVALabGC
{
    class CalibrateForm : Form1
    {
        ...
        //tobii support WORK AROUND
        //((Tobii.TobiiReader)this.context.Reader).Calibrate();

        public override void FormScreen()
        {
            if (context.Reader is Tobii.TobiiReader)
            {
                ((Tobii.TobiiReader)context.Reader).Calibrate();
            }
            else
            {
                base.FormScreen();
        ...
        public override void FullScreen(Screen scrn)
        {
            if (context.Reader is Tobii.TobiiReader)
            {
                ((Tobii.TobiiReader)context.Reader).Calibrate(scrn);
            }
            else
            {
