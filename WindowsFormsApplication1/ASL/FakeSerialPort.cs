...
namespace EVALabGC.ASL
{
    class FakeSerialPort
    {
        ...
        {
            string filename = Application.StartupPath +"\\ASL\\DATA.txt";
            ...
            while ((str = reader.ReadLine()) != null)
            {
                ...
                while (line[j].Trim().Length <= 0) j++;
                xy[0] = Int32.Parse(line[j]);
                j++;
                while (line[j].Trim().Length <= 0) j++;
                xy[1] = Int32.Parse(line[j]);
                datas.Add(xy);
            }
        ...
        public void DiscardInBuffer()
        {
            ...
            if (x > Context.WIDTH) x = 1;
            if (y > Context.HEIGHT) y = 1;
            int p = 40;
            dataBOut[1] = (byte)(p >> 8);
            dataBOut[2] = (byte)(p);
            dataBOut[4] = (byte)(x >> 8);
        ...
        {
            int T = 1400 + 500;
            int startX = (int)(200.0 / 1024.0 * Context.WIDTH);
            int startY = (int)(600.0 / 768.0 * Context.HEIGHT);
            int targetX = (int)(800.0 / 1024.0 * Context.WIDTH);
            int targetY = (int)(600.0 / 768.0 * Context.HEIGHT);
            timer.Stop();
            timerGlobal.Stop();
